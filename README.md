...
- 🎯 **Agent-Specific Views**: Filter by `--opencode`, `--claude`, or `--agy` (combinable with `--today` and `--json`).
- 📊 **Session Tracking**: Displays the most recently active sessions for OpenCode (both scopes) and Antigravity, with turn counts where available (AGY generation turns).
...
│   ├── cli.py               # CLI parser (--today/--opencode/--claude/--agy/--json/--no-color/--version) + fault-isolated dispatch
...
│   └── test_cli.py          # Version flag, failure isolation, JSON + empty-state behavior
